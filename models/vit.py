...
from .vitutil import DropPath, to_2tuple, trunc_normal_
...
urls = {
    ...
    'vit_huge_patch16_224': None,
    'vit_huge_patch32_384': None,
    # hybrid models
...
class PatchEmbed(nn.Module):
   ...
   def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
       super().__init__()
       new_patch_size = to_2tuple(patch_size // 4)
       ...
       self.num_patches = num_patches
       print(img_size)
       if img_size == (32, 32):  # for cifar
           print('cifar')
           self.conv1 = nn.Conv2d(in_chans, 64, kernel_size=3, stride=2, padding=1, bias=False)  # 16x16
           self.conv2 = nn.Conv2d(64, 64, kernel_size=3, stride=2, padding=1, bias=False)  # 14x14
           self.proj = nn.Conv2d(64, embed_dim, kernel_size=1, stride=1) # 14x14

       elif img_size == (224, 224):  # for imagenet
           print('imagenet')
           self.conv1 = nn.Conv2d(in_chans, 64, kernel_size=7, stride=2, padding=3, bias=False)  # 112x112
           self.conv2 = nn.Conv2d(64, 64, kernel_size=3, stride=2, padding=1, bias=False)  # 56x56
           self.proj = nn.Conv2d(64, embed_dim, kernel_size=new_patch_size, stride=new_patch_size)  # 14x14
   ...
   def forward(self, x):
       ...
       x = x.flatten(2).transpose(1, 2)  # [B, N=W*H, C]
       return x
...
def vit_vsmall_patch16_224(pretrained=False, **kwargs):
    model = VisionTransformer(patch_size=16, embed_dim=384, depth=12, num_heads=6, mlp_ratio=3., **kwargs)
    return model
